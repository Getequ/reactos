...
VOID SetItemBehavior(HWND hwnd, HWND hCombo)
{
    ...
    HWND hListView = GetDlgItem(hwnd, IDC_NOTIFICATION_LIST);
    CToolbar<InternalIconData>* toolbar = (CToolbar<InternalIconData>*)GetWindowLongPtrW(hListView, GWLP_USERDATA);
    ...
    toolbar->GetButtonInfo(iItem, &tbbi);
    ...
    toolbar->SetButtonInfo(iItem, &tbbi);
    //toolbar->AutoSize();
    
    NMHDR nmh = {toolbar->GetParent().GetParent(), 0, NTNWM_REALIGN};
    toolbar->GetParent().GetParent().SendMessage(WM_NOTIFY, 0, (LPARAM) &nmh);
